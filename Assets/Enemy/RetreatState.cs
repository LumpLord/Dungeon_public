...
public class RetreatState : EnemyCombatStateBase
{
    ...
    {
        if (debugEnabled) Debug.Log($"{controller.name} EnterState() called for {this.GetType().Name}");
        if (debugEnabled) Debug.Log($"{controller.name} is entering {this.GetType().Name}");
        ...
        {
            if (debugEnabled) Debug.LogWarning($"{controller.name} RetreatState early exit: agent null? {agent == null}, target null? {target == null}");
            return;
        }
        ...
        if (debugEnabled) Debug.Log($"{controller.name} Retreat destination calculated: {retreatDestination}");
        ...
        {
            if (debugEnabled) Debug.LogWarning($"{controller.name} RetreatState aborted: destination too close to retreat.");
            endTime = Time.time; // Immediately end the state
            return;
        }
        agent.SetDestination(retreatDestination);

        if (debugEnabled) Debug.Log($"{controller.name} Retreat destination set: {agent.destination}, agent speed: {agent.speed}");

        if (debugEnabled) Debug.Log($"{controller.name} RetreatState: setting endTime with minTimeBeforeNextState = {minTimeBeforeNextState}");
        if (minTimeBeforeNextState <= 0f)
        {
            minTimeBeforeNextState = 1.25f; // fallback default
            if (debugEnabled) Debug.LogWarning($"{controller.name} RetreatState: minTimeBeforeNextState was 0 or less, defaulting to 1.25s");
        }
        endTime = Time.time + minTimeBeforeNextState;
        if (debugEnabled) Debug.Log($"{controller.name} RetreatState endTime set to: {endTime}, current time: {Time.time}");
    }

    public override IEnumerator Execute(EnemyCombatController controller)
    {
        if (debugEnabled) Debug.Log($"{controller.name} is executing {this.GetType().Name}");
        if (debugEnabled) Debug.Log($"{controller.name} RetreatState duration set to end at: {endTime}, current time: {Time.time}");
        var agent = controller.GetAgent();
        var target = controller.GetTarget();
        ...
        while (true)
        {
            ...
            {
                if (debugEnabled) Debug.Log($"{controller.name} RetreatState destination reached. Exiting early.");
                break;
            }
        ...
        {
            if (debugEnabled) Debug.LogWarning($"{controller.name} RetreatState completed but agent.updatePosition was disabled. Resetting...");
            ...
            if (NavMesh.SamplePosition(controller.transform.position, out hit, 1.0f, NavMesh.AllAreas))
            {
                ...
                if (debugEnabled) Debug.Log($"{controller.name} RetreatState: agent warped to valid navmesh position at {hit.position}");
            }
            else
            {
                if (debugEnabled) Debug.LogWarning($"{controller.name} RetreatState: failed to find valid navmesh position near {controller.transform.position}");
            }
        }

        if (debugEnabled) Debug.Log($"{controller.name} RetreatState duration met or destination reached. Exiting.");
    }

    public override void ExitState(EnemyCombatController controller)
    {
        if (debugEnabled) Debug.Log($"{controller.name} is exiting {this.GetType().Name}");
        var agent = controller.GetAgent();
        if (agent != null)
            agent.speed /= retreatSpeedMultiplier;
    }
